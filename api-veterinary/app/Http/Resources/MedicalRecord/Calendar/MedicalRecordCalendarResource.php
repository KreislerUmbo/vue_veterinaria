...
class MedicalRecordCalendarResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        if ($this->resource->appointment_id) {
            $resource = $this->resource->appointment; // relacion con cita medica
            $calendar = "Appointment";
        }
        if ($this->resource->vaccination_id) {
            $resource = $this->resource->vaccination; // relacion con vacunacion
            $calendar = "Vaccination";
        }
        if ($this->resource->surgerie_id) {
            $resource = $this->resource->surgerie; // relacion con cirugia
            $calendar = "Surgeries";
        }

       // error_log(json_encode($resource));

        $hour_star = $resource->schedules->first()->schedule_hour->hour_start;
        $hour_end = $resource->schedules->first()->schedule_hour->hour_end;
        return [
            "id" => $this->resource->id,
            "title" => $resource->pet->name,
            "start" => Carbon::parse(Carbon::parse($this->resource->event_date)->format("Y-m-d") . ' ' . $hour_star)->format("Y-m-d H:i:s"), // fecha y hora de inicio pero del registro medico
