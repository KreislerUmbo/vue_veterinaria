...
export const setupGuards = router => {
 ...
 /*    if (to.meta.not_autentication == false) {
      return true;
    } */
    ...
    if (USER && USER.role.name != 'Super-Admin') {
      ...
      //validamos si el permiso que tiene la ur a la que vamos a ingresar es parte de los permisos del usuario autenticado
     // if (permissions.includes(to.meta.permission) || to.meta.permission == "all") {
        ...
        //caso contraro no tenga los permisos para ingresar a la vista entonces lo mandaremos a la url not-authoired
        return { name: 'not-authorized' };
      }
    }
    if (!isLoggedIn && to.matched.length) {//!canNavigate(to) &&
