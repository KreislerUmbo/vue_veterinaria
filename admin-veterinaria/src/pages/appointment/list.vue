...
const state_appointment = ref(null); // 1: pendiente, 2: cancelado, 3: atendido
...
const editItem = (item) => {
    router.push({ name: 'appointment-edit-id', params: { id: item.id } });
}
...
const deleteItem = (item) => {
    appointment_selected_deleted.value = item;// Seleccionar la cita a eliminar
    isDeleteAppointmentDialogVisible.value = true;// Abrir el diálogo
}
...
<template>
    <div>
        <VCard title="📅Citas Médicas">
            <VCardText class="d-flex flex-wrap gap-4">
                <VRow>
                    ...
                    <VCol cols="2">
                        ...
                        ]" v-model="state_appointment" label="Estado de la  Cita" item-title="name" item-value="id"
            ...
            <DeleteAppointmentDialog v-if="appointment_selected_deleted"
