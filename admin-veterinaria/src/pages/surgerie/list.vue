...
const surgerie_type = ref(['ESTERELIZACIÓN', 'CASTRACIÓN', 'TRAUMATÓLOGICAS', 'OCULARES', 'ONCOLOGICAS', 'OTROS']);
...
const downloadExcel = () => {
    ...
    window.open(import.meta.env.VITE_API_BASE_URL + '/surgerie-excel?k=1' + LINK, '_blank');
}

const editItem = (item) => {
    router.push({ name: 'surgerie-edit-id', params: { id: item.id } });
}



const deleteSurgerie = (item) => {
    let INDEX = surgeries.value.findIndex((surgerie) => surgerie.id == item.id);
    if (INDEX != -1) {
...
const deleteItem = (item) => {
    surgerie_selected_deleted.value = item;// Seleccionar la cita a eliminar
    isDeleteSurgerieDialogVisible.value = true;// Abrir el diálogo
}
...
watch(isDeleteSurgerieDialogVisible, (val) => {// Cuando se cierre el diálogo, limpiar la cita seleccionada
    console.log(val);
    if (val == false) {
            ...
            <VCardText class="d-flex flex-wrap gap-4">
                <VRow>
                    ...
                    <VCol cols="2">
                        ...
                        ]" v-model="state_surgerie" label="Estado de la  cirugia" item-title="name" item-value="id"
                            placeholder="Select Estado cirugia" eager />
                    </VCol>
                    ...
                    <VCol cols="3">
                        <VBtn color="primary" prepend-icon="ri-add-line" @click="router.push({ name: 'surgerie-add' })">
                            Agregar Cirugía
                        </VBtn>
                    </VCol>
                    <VCol cols="2">
