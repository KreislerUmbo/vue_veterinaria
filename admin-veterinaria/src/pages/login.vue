  ...
  try {
   ...
   //const resp = await $fetch('/api/auth/login',{
      ...
      onResponseError({ response }) {
        console.log(response?._data?.error || 'Unknown error');
    ...
    success_exist.value = true;

setTimeout(async()=>{//setTimeout es para darle tiempo y que slaga el mensaje verde datos correctos
  await nextTick(() => {
    ...
    <VCol cols="12" md="4" class="auth-card-v2 d-flex align-center justify-center"
      ...
      <VCard flat :max-width="500" class="mt-12 mt-sm-0 pa-5 pa-lg-7">
        <VCardText>
          ...
          <p class="mb-0">
            Inicie sesión en su cuenta y comienza la aventura
          </p>
        </VCardText>
          ...
          <VForm @submit.prevent="login()">
            <VRow>
              ...
              <VCol cols="12">
                ...
                <VAlert type="error" class="my-2" v-if="error_exist">
                  Error presentado en: <strong>{{ error_exist }}</strong>
                </VAlert>
                <VAlert type="success" class="my-2" v-if="success_exist">
