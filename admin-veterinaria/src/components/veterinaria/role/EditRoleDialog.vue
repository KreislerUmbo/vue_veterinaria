...
const store = async () => {
  ...
  let data = {
    name: role.value,
    permissions: permissions.value, //permissions: este nombre debe ir igual en el backend
  }
  ...
  try {
    ...
    if (resp.message == 403) {
      ...
      success.value = "EL ROL SE HA EDITADO CON EXITO";
      emit('addRole', true)
    }
...
onMounted(() => {
  ...
  permissions.value=role_selected.value.permission_pluck;
})
    ...
    <VCard class="refer-and-earn-dialog pa-3 pa-sm-11">
      ...
      <VCardText class="pa-5">
        <div class="mb-6">
          <h4 class="text-h4 text-center mb-2" v-if="role_selected">
            Modificar Roles: {{ role_selected.id }}
          </h4>
        </div>
      ...
      <VCardText class="pa-5">
        ...
        <VTable>
          ...
          <tbody>
            <tr v-for="(item, index) in LIST_PERMISSION" :key="item.name">
              <td>
                {{ item.name }}
              </td>
              <td>
