...
const update = async () => {
    ...
    try {
        ...
        if (resp.message == 403) {
            ...
            success.value = "EL PAGO SE HA EDITADO EXITOSAMENTE";
        }
        setTimeout(() => {
            success.value = null;
            warning.value = null;
            ...
            emit('editPayment', resp.payment)//addPayment viene del padre list.vue y resp.payment es el nuevo pago creado
        }, 1500);
...
onMounted(() => {
    console.log(props.medicalRecord, props.paymentSelected); //solo para debug muestra los props recibidos del padre que son datos del pago a editar y la historia clinica
    if (props.paymentSelected) {
        ...
        form.value.notes = props.paymentSelected.notes;
        form.value.date_payment = props.paymentSelected.date_payment;
        // el props es reactivo y se actualiza cuando se abre el dialog y es propia del componente EditPaymentDialog.vue
        // podemos decir que es como el "data" del componente padre list.vue
    }
});
</script>
            ...
            <VCardText class="pa-5">
                <div class="mb-6">
                    <h4 class="text-h4 text-center mb-2">
                        EDITAR PAGOS: {{ props.paymentSelected.id }}
                    </h4>
                </div>
                <VRow>
                    <VCol cols="4">
